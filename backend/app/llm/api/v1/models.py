...
@router.get(
    '/available',
    summary='获取可用模型列表',
    description='公开接口，获取所有启用的模型列表（不含敏感信息）',
)
async def get_available_models(db: CurrentSession) -> ResponseSchemaModel[list[GetAvailableModel]]:
...
) -> ResponseSchemaModel[list[GetModelConfigList]]:
    data = await model_service.get_list(
        db,
